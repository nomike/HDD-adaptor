...
drive_heigth = 9; // The height of the drive in mm. Only relevant if you want to stack another adaptor on top of this one. Typical values are: 5mm, 7mm, 9.5mm, 12.5mm, 15mm, 19mm
sliders = "big"; // "big", small", or "none"
...
wall_width = 6; // 6mm should be fine. More just increases the print time and filament usage.
...
drive_hole_diameter = 3; // Fits a standard M2.5 screw including a little bit of wiggle room
...
small_slider_offset = 7.4 + (small_slider_diameter / 2);
small_slider_distance = 88.6;

connecting_nut_diameter = 5.4;
connecting_nut_heigth = 2.3;
connecting_screwhead_diameter = 5.5;
connecting_screwhead_heigth = 2;
...
union() {
    difference() {
        ...
        // translate([0, wall_width + (sqrt(2* ((cutout3_length1 / 2) * (cutout3_length1 / 2))) / 2), 0]) rotate([0, 0, 45]) cube([_angle_side, _angle_side, bed_heigth * 2], center = true);

        // translate([cutout3_length1 / 2, _cutout3_y_inner_offset + cutout3_width2, 0]) rotate([0, 0, 45]) cube([5, 5, bed_heigth * 5], center = true);
